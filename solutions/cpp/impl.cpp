...
//
// В секции "вспомогательный код" внизу файла уже написан код, который:
// 1. Читает .wav файл, путь к которому указан в переменной INPUT_FILE
// 2. Преобразует аудио данные в удобный вид
// 3. Вызывает функцию process, которая должна обработать сигнал и вернуть
//    результат обработки
// 4. Проверяет результат на корректность (количество каналов, равное количество
//    сэмплов в канале и т.д.)
// 5. Записывает результат в .wav файл, путь к которому указан в переменной
//    OUTPUT_FILE
//
// Как сделать новый фильтр:
// 1. Определить новую функцию, используя make_quiet как пример
...
const char *const INPUT_FILE = "test-sound.wav";
const char *const OUTPUT_FILE = "result.wav";

// Пример функции для обработки сигнала.
// Умножает все сэмплы на заданный множитель
vector<vector<double> > multiply_amplitude(const vector<vector<double> > &channels, double multiplier) {
    vector<vector<double>> result(channels.size());
    for (int k = 0; k < result.size(); ++k) {
        result[k].resize(channels[k].size());
    }

    // Код для обработки аудио-сигнала. Исполняется независимо для каждого
    // канала
    for (int k = 0; k < result.size(); ++k) {
        for (int i = 0; i < result[k].size(); ++i) {
            result[k][i] = channels[k][i] * multiplier;
        }
    }
...
vector<vector<double> > trim(const vector<vector<double> > &channels) {
    vector<vector<double>> result(channels.size());
    for (int k = 0; k < result.size(); ++k) {
        result[k].resize(channels[k].size());
    }
    ...
    for (int k = 0; k < result.size(); ++k) {
        for (int i = 0; i < result[k].size(); ++i) {
            result[k][i] = min(max(-1.0, (double)channels[k][i]), 1.0);
        }
    }
...
// Функция process вызывается вспомогательным кодом для обработки сигнала
//
// Аргументы:
// channels - Список каналов. Каждый канал - список сэмплов. 
// channels[ch][i] - сэмпл i в канале ch.
vector<vector<double> > process(const vector<vector<double> > &channels, int sample_rate) {
    // Здесь вызывайте свою функцию
    return trim(multiply_amplitude(channels, 2));
}

// Эти функции определены ниже в файле
void read_wav(const string &filename, vector<vector<double> > *channels, int *sample_rate, int *sample_width);
void write_wav(const string &filename, const vector<vector<double> > &channels, int sample_rate, int sample_width);
bool check_result(const vector<vector<double> > &channels);
...
int main() {
    ...
    read_wav(INPUT_FILE, &channels, &sample_rate, &sample_width);

    cout << "Обработка... ";
    vector<vector<double> > channels_out = process(channels, sample_rate);
    if (!check_result(channels_out)) {
        return 0;
    }
    cout << "Готово" << endl;
...
////////////////////////////////////////////////////////////////////////////////
// Ниже этой строки идёт вспомогательный код
////////////////////////////////////////////////////////////////////////////////


////////////////////////////////////////////////////////////////////////////////
// Начало заголовочного файла библиотеки для чтения и записи .wav файлов
////////////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////////////
// Конец заголовочного файла библиотеки для чтения и записи .wav файлов
////////////////////////////////////////////////////////////////////////////////


////////////////////////////////////////////////////////////////////////////////
// Вспомогательные функции для чтения и записи .wav файлов
////////////////////////////////////////////////////////////////////////////////
...
void read_wav(const string &filename, vector<vector<double> > *channels, int *sample_rate, int *sample_width) {
    ...
    // vector<vector<double> > samples_copy(audioFile.samples.begin(), audioFile.samples.end());
    channels->assign(audioFile.samples.begin(), audioFile.samples.end());
    *sample_rate = audioFile.getSampleRate();
    *sample_width = audioFile.getBitDepth();

    cout << "Количество каналов: " << audioFile.getNumChannels() << endl;
    cout << "Размер сэмпла: " << *sample_width << endl;
    cout << "Частота сэмплирования: " << *sample_rate << endl;
    cout << "Количество фреймов: " << audioFile.getNumSamplesPerChannel() << endl;
}
...
void write_wav(const string &filename, const vector<vector<double> > &channels, int sample_rate, int sample_width) {
    ...
    cout << "Запись файла... ";
    vector<vector<double> > channels_copy(channels.begin(), channels.end());
    audioFile.setAudioBuffer(channels_copy);
    audioFile.setSampleRate(sample_rate);
    audioFile.setBitDepth(sample_width);
...
bool check_result(const vector<vector<double> > &channels) {
    if (channels.size() == 0) {
        cout << "Ошибка: результат обработки содержит 0 каналов" << endl;
        return false;
    }

    int len_0 = channels[0].size();
    for (int i = 1; i < channels.size(); ++i) {
        int len_i = channels[i].size();
        if (len_0 != len_i) {
            cout << "Ошибка: количество сэмплов в каналах 1 (" << len_0 << ") и " << i + 1 << " (" << len_i << ") не совпадает" << endl;
            return false;
        }
    }
...
////////////////////////////////////////////////////////////////////////////////
// Начало файла с реализацией библиотеки для чтения и записи .wav файлов
////////////////////////////////////////////////////////////////////////////////
